...
const BookDetailsPage: React.FC = () => {
  ...
  const [summarySource, setSummarySource] = useState<string | null>(null); // 'ai' | 'cached' | 'fallback'
  ...
  const fetchAiSummary = async (bookData: any) => {
    if (!bookData || !bookData.books_id) return;

    setSummaryLoading(true);
    setSummarySource(null);
    try {
      // Gọi API book-summary - backend sẽ tự lấy nội dung sách và tóm tắt
      const summaryResponse = await book_summary({
        books_id: bookData.books_id || bookData.id,
      });

      if (summaryResponse.success && summaryResponse.summary) {
        setAiSummary(summaryResponse.summary);
        // Xác định nguồn summary
        if (summaryResponse.cached) {
          setSummarySource("cached");
        } else if (summaryResponse.model_used === "simple_fallback") {
  ...
  const resolveImageSrc = useMemo(
    () =>
      (b: any): string => {
        ...
        const raw =
          (b.coverUrl as string) || (b.image as string) || "/logo/logo.svg";
      ...
      try {
        ...
        if (res && res.success && Array.isArray(res.data)) {
          ...
          const found = res.data.find(
            (b: any) => String(b.books_id ?? b.id) === currentIdStr
          );
          ...
          if (!found) {
            ...
            // Lấy AI summary từ file tài liệu
            fetchAiSummary(found);
            ...
            const related = res.data.filter((b: any) => {
              ...
              return (
                (bAuthor && bAuthor === currentAuthor) ||
                (bType && bType === currentType)
              );
            });
  ...
  const toggleFav = (id: number | string | undefined) => {
    ...
    setFavorites((prev) => {
      ...
      try {
        localStorage.setItem("favoriteBooks", JSON.stringify(Array.from(next)));
      } catch {}
      return next;
    });
  };
  ...
  const handleBorrowBook = async () => {
    ...
    try {
      ...
      if (response.success) {
        message.success("Đã gửi yêu cầu mượn sách! Vui lòng chờ xét duyệt.");
      } else {
        message.error(response.message || "Có lỗi xảy ra khi gửi yêu cầu mượn sách");
      }
    } catch (error: any) {
      console.error("Borrow error:", error);
      message.error(error?.message || "Có lỗi xảy ra khi gửi yêu cầu mượn sách");
    } finally {
      setBorrowing(false);
    }
  };
  ...
  const description =
    book?.Description || book?.description || "Chưa có mô tả cho tài liệu này.";
  const author =
    book?.Author || book?.author || book?.AUTHOR || "Chưa cập nhật";
          ...
          <Col xs={24} md={16} lg={17}>
            ...
            <div style={{ marginBottom: 32 }}>
              <Space align="center" style={{ marginBottom: 8 }}>
                ...
                >
                  {book.DocumentType || book.documentType || "Tài liệu"}
                </Tag>
                <Space>
                  <Rate
                    allowHalf
                ...
                items={[
                  {
                    ...
                    children: (
                      <div style={{ padding: "32px" }}>
                        {/* Hiển thị AI Summary hoặc Description */}
                        {summaryLoading ? (
                          <div
                            style={{
                              marginBottom: 24,
                        ...
                        <Row gutter={[24, 24]}>
                          ...
                          <Col span={12}>
                            <Space direction="vertical" size={4}>
                              ...
                              <Text strong style={{ fontSize: 16 }}>
                                <CalendarOutlined />{" "}
                                {book.PublishYear || book.publishYear || "N/A"}
                              </Text>
                            </Space>
                          </Col>
                          <Col span={12}>
                  ...
                  },
                  {
                    ...
                    label: (
                      <span style={{ fontSize: 16, fontWeight: 500 }}>
                        <MessageOutlined /> Đánh giá ({reviews.length})
                      </span>
                    ),
                    children: (
                      <div style={{ padding: "32px" }}>
          ...
          <div style={{ marginTop: 40, marginBottom: 40 }}>
            ...
            <List
              ...
              renderItem={(item) => (
                <List.Item>
                  <Link
                    href={`/books/details?books_id=${item.books_id ?? item.id}`}
      ...
      >
        ...
        {pdfUrl ? (
          <object
            ...
            width="100%"
            height="100%"
            style={{ border: "none", height: "calc(85vh - 50px)" }}
          >
            <div
              style={{
                display: "flex",
